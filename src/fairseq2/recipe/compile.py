...
def _compile_model(model: Model, options_section: CompileOptionsSection) -> None:
    if not model.handler.supports_compilation:
        raise TorchCompileNotSupportedError(model.name)
...
class TorchCompileNotSupportedError(Exception):
    def __init__(self, model_name: str) -> None:
        super().__init__(f"{model_name} model does not support torch.compile().")
