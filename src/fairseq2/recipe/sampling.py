...
from fairseq2.recipe.config import SamplerNotKnownError, SamplingConfig
...
class SamplingFactory:
    ...
    def create_seq_generator(self, config: SamplingConfig) -> SequenceGenerator:
        try:
    ...
    def create_seq2seq_generator(self, config: SamplingConfig) -> Seq2SeqGenerator:
        try:
            sampler = self._component_manager.create_component(
                Sampler, config.sampler.name, config.sampler.config
            )
