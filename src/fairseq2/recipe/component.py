...
@final
class ResolverBackedComponentManager(ComponentManager):
    ...
    def create_component(self, kls: type[T], name: str, config: object) -> T:
        try:
            entry = self._resolver.resolve(_ComponentEntry, key=(kls, name))
        except DependencyNotFoundError:
            raise UnknownComponentError(name, kls) from None

        if not isinstance(config, entry.config_kls):
            raise TypeError(
                f"`config` is expected to be of type `{entry.config_kls}`, but is of type `{type(config)}` instead."
            )
...
class UnknownComponentError(Exception):
    ...
    def __init__(self, name: str, kls: type[object]) -> None:
...
def _create_component_manager(resolver: DependencyResolver) -> ComponentManager:
    value_converter = resolver.resolve(ValueConverter)

    return ResolverBackedComponentManager(resolver, value_converter)
