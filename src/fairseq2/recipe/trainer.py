...
def _create_trainer(
    ...
    if gangs.root.device.type == "cpu":
        log.warning("Based on the environment setup, training will be run on CPU. If this was not intended, check your job options (e.g. pass `--gpus-per-node` on Slurm).")  # fmt: skip
...
class HuggingFaceNotSupportedError(Exception):
    def __init__(self, model_name: str) -> None:
        super().__init__(
            f"{model_name} model does not support Hugging Face conversion."
        )
...
def _maybe_get_score_metric_descriptor(
    ...
    try:
        ...
        raise MetricNotKnownError(score_metric) from None
...
def _create_garbage_collector(resolver: DependencyResolver) -> GarbageCollector:
    ...
    return StandardGarbageCollector(trainer_section.gc_every_n_steps)
