...
from typing import Any, Final, Generic, Mapping, Sequence, TypeVar, final
...
from fairseq2.optim.dynamic_loss_scaler import DynamicLossScaler
...
from fairseq2.recipe.device import create_device_stat_tracker
from fairseq2.recipe.early_stopper import EarlyStopper
...
from fairseq2.recipe.metric_recorders import create_metric_recorder
from fairseq2.recipe.metrics import extend_batch_metric_values
from fairseq2.recipe.model import Model
from fairseq2.recipe.profilers import create_profiler
from fairseq2.recipe.task import Task, TaskError, TaskStopException
from fairseq2.recipe.validator import Validator
...
def create_trainer(
    ...
    score_metric_descriptor = maybe_get_score_metric_descriptor(resolver)

    checkpoint_manager = resolver.resolve(CheckpointManager)

    hugging_face_saver = maybe_create_hugging_face_saver(resolver)

    metric_recorder = create_metric_recorder(resolver)

    garbage_collector = create_garbage_collector(resolver)

    profiler = create_profiler(resolver)

    device_stat_tracker = create_device_stat_tracker(resolver)
...
def maybe_create_hugging_face_saver(
    resolver: DependencyResolver,
    ...
    if not model.handler.supports_hugging_face:
        raise HuggingFaceNotSupportedError(model.name)
...
def maybe_get_score_metric_descriptor(
    resolver: DependencyResolver,
    ...
    try:
        return metric_descriptors.resolve(score_metric)
    except LookupError:
        raise UnknownMetricDescriptorError(score_metric) from None


def create_garbage_collector(resolver: DependencyResolver) -> GarbageCollector:
    trainer_section = get_config_section(resolver, "trainer", TrainerSection)
