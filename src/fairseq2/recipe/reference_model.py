...
def setup_eval_model(resolver: DependencyResolver) -> Model:
    model = resolver.resolve(Model, key="base_eval")

    model_section = get_recipe_config_section(resolver, "model", ReferenceModelSection)

    return _do_setup_reference_model(resolver, model, model_section)


def setup_generator_model(resolver: DependencyResolver) -> Model:
    model = resolver.resolve(Model, key="base_generator")
...
class _ReferenceModelLoader:
    ...
    def load(
        ...
        try:
            if gangs.dp.rank == 0:
                module = handler.load(resolver, card, gangs, dtype, config)
            else:
