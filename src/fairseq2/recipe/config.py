...
from fairseq2.recipe.config_preparer import SupportsStructure
...
@dataclass(kw_only=True)
class SamplerSection(SupportsStructure):
...
@dataclass(kw_only=True)
class BeamSearchAlgorithmSection(SupportsStructure):
...
def get_config_section(
    ...
    *,
    ...
    if not isinstance(section, kls):
        raise TypeError(
            f"{section} recipe configuration section must be of type `{kls}`, but is of type `{type(section)}` instead."
        )
