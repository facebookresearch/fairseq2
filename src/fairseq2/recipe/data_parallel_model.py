...
from fairseq2.recipe.base_model import prepare_base_model
from fairseq2.recipe.checkpoint import check_has_checkpoint
from fairseq2.recipe.config import TrainerSection, get_recipe_config_section
...
def setup_data_parallel_model(resolver: DependencyResolver) -> Model:
    model = resolver.resolve(Model, key="base")

    prepare_base_model(resolver, model)
    ...
    try:
        ...
        if data_parallelism == "fsdp":
            model = _wrap_fsdp(resolver, model, gangs)
...
def _wrap_ddp(model: Model, gangs: Gangs) -> Model:
    if gangs.dp.size == 1:
    ...
    # though and can bring measurable improvement.
...
def _wrap_fsdp(resolver: DependencyResolver, model: Model, gangs: Gangs) -> Model:
