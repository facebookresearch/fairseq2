...
from fairseq2.recipe.config import BeamSearchAlgorithmNotKnownError, BeamSearchConfig


@final
class BeamSearchFactory:
    def __init__(
        self, model: Model, tokenizer: Tokenizer, component_manager: ComponentManager
    ) -> None:
        self._model = model
        self._tokenizer = tokenizer
        self._component_manager = component_manager

    def create_seq_generator(self, config: BeamSearchConfig) -> SequenceGenerator:
        try:
    ...
    def create_seq2seq_generator(self, config: BeamSearchConfig) -> Seq2SeqGenerator:
        try:
            algo = self._component_manager.create_component(
                BeamSearchAlgorithm, config.algo.name, config.algo.config
            )
