...
from fairseq2.model.sharder import ShardSpec
...
class DelegatingModelCheckpointLoader(ModelCheckpointLoader):
    _loaders: Collection[ModelCheckpointLoader]
    _file_system: FileSystem

    def __init__(
        self, loaders: Collection[ModelCheckpointLoader], file_system: FileSystem
    ) -> None:
...
@final
class SafetensorsModelCheckpointLoader(ModelCheckpointLoader):
    _file_system: FileSystem
...
def _access_error(path: Path) -> InfraError:
    return InfraError(
        f"A system error has occurred while accessing the '{path}' path. See the nested exception for details."
    )


def _create_model_checkpoint_loader(
    resolver: DependencyResolver,
) -> ModelCheckpointLoader:
    file_system = resolver.resolve(FileSystem)

    other_loaders = resolver.resolve_all(ModelCheckpointLoader, key="alt")

    tensor_loader = TorchTensorLoader(file_system)

    safetensors_loader = HuggingFaceSafetensorsLoader(file_system)

    basic_loader = BasicModelCheckpointLoader(file_system, tensor_loader)

    st_loader = SafetensorsModelCheckpointLoader(file_system, safetensors_loader)

    llama_loader = LLaMACheckpointLoader(file_system, tensor_loader)

    native_loader = NativeModelCheckpointLoader(file_system, tensor_loader)

    loaders = [basic_loader, st_loader, llama_loader, native_loader]

    loaders.extend(other_loaders)
