...
def apply_layerwise_activation_checkpointing(
    ...
    applied = _do_apply_layerwise_ac(model, every_nth_layer)

    if not applied:
        raise ValueError("`model` must contain at least one layer stack.")


def _do_apply_layerwise_ac(module: Module, every_nth_layer: int) -> bool:
    applied = False
    ...
    for child_name, child in children:
        if isinstance(child, LayerStack):
            ...
            if _do_apply_layerwise_ac(child, every_nth_layer):
                applied = True
