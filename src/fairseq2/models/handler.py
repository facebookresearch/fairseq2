...
from fairseq2.models.checkpoint import CheckpointError, CheckpointLoader
...
class DelegatingModelHandler(ModelHandler):
    ...
    def get_arch_config(self, arch: str | None) -> object:
        ...
        try:
            return self._configs.resolve(effective_arch)
        except LookupError:
            if arch is None:
                raise ContractError(
    ...
    def create(
        ...
        config = structure(config, self._config_kls)

        validate(config)

        return self._do_create(resolver, config, gangs, dtype, meta)
        ...
        *,
        ...
        try:
            return self.load_from_path(
                resolver, path, name, config, gangs, dtype, mmap=mmap, restrict=restrict
            )
...
def register_model_family(
    ...
    *,
    ...
    def create_handler(resolver: DependencyResolver) -> ModelHandler:
        asset_download_manager = resolver.resolve(AssetDownloadManager)

        checkpoint_loader = resolver.resolve(CheckpointLoader)

        sharder = resolver.resolve(ModelSharder)

        progress_reporter = resolver.resolve(ProgressReporter)

        configs = resolver.resolve_provider(config_kls)
    ...
    container.register(ModelHandler, create_handler, key=family)
