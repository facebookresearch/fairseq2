...
# llama3 chat template with assistant mask support, see
# https://github.com/huggingface/transformers/issues/28950
LLAMA3_CHAT_TEMPLATE: Final = """{{- bos_token }}
...
def load_llama_tokenizer(path: Path, config: LLaMATokenizerConfig) -> Tokenizer:
    if config.use_v2_tokenizer:
        is_dir = path.is_dir()
        if is_dir:
            return _load_llama3_hg_tokenizer(path, config)

        return _load_llama3_tokenizer(path, config)
...
def _load_llama3_tokenizer(path: Path, config: LLaMATokenizerConfig) -> Tokenizer:
...
def _load_llama3_hg_tokenizer(path: Path, config: LLaMATokenizerConfig) -> Tokenizer:
    eos_token = "<|eot_id|>" if config.use_eot else "<|end_of_text|>"
