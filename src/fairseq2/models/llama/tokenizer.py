...
def load_llama_tokenizer(
    resolver: DependencyResolver, path: Path, card: AssetCard
) -> Tokenizer:
    try:
        ...
        raise tokenizer_asset_card_error(card.name) from ex
    ...
    if use_v2:
        try:
            ...
            raise TokenizerLoadError(
                card.name, f"The path of the '{card.name}' tokenizer cannot be accessed. See the nested exception for details."  # fmt: skip
            ) from ex

        if is_dir:
            return _load_llama3_hg_tokenizer(path, card)

        return _load_llama3_tokenizer(path, card)

    return load_basic_sentencepiece_tokenizer(resolver, path, card)


def _load_llama3_tokenizer(path: Path, card: AssetCard) -> Tokenizer:
    ...
    try:
        ...
        raise TokenizerLoadError(
            card.name, f"The '{card.name}' tokenizer model cannot be loaded. See the nested exception for details."  # fmt: skip
        ) from ex

    return LLaMA3Tokenizer(model, eos_token)


def _load_llama3_hg_tokenizer(path: Path, card: AssetCard) -> Tokenizer:
    try:
        use_eot = card.field("use_eot").as_(bool)
    except AssetCardFieldNotFoundError:
        use_eot = False
