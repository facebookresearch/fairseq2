...
try:
    from mamba_ssm.ops.triton.selective_state_update import (  # type: ignore[import-untyped]
        selective_state_update,
    )
    from mamba_ssm.ops.triton.ssd_combined import (  # type: ignore[import-untyped]
...
class NemotronHMamba2Mixer(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.projection_size = (
            self.intermediate_size + self.conv_dim + num_heads
        )  # 10304
    ...
    def _init_parameters(self, time_step_min: float, time_step_max: float) -> None:
    ...
    def _forward_incremental(
        ...
        if HAS_MAMBA_SSM and x.is_cuda:
            ...
            A_expanded = (
                A.unsqueeze(-1).unsqueeze(-1).expand(-1, self.head_dim, self.state_size)
            )  # [H] → [H,D,N]
            D_expanded = self.D.unsqueeze(-1).expand(-1, self.head_dim)  # [H] → [H,D]
            dt_bias_expanded = (
                self.dt_bias.float().unsqueeze(-1).expand(-1, self.head_dim)
            )  # [H] → [H,D]
            ...
            dBx = torch.einsum("bhd,bhn->bhdn", x * dt_val.unsqueeze(-1), B_val)
        ...
        return output.unsqueeze(1)  # type: ignore[no-any-return]  # [B, 1, model_dim]
