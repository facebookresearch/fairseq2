...
class NemotronHExpert(nn.Module):
    ...
    @override
    def forward(self, x: Tensor) -> Tensor:
        out: Tensor = self.down_proj(self.act(self.up_proj(x)))
        return out
...
class NemotronHTopKRouter(nn.Module):
    ...
    def forward(self, hidden_states: Tensor) -> tuple[Tensor, Tensor]:
        ...
        scores_for_choice = router_logits + self.e_score_correction_bias  # type: ignore[operator]
...
class NemotronHMoE(nn.Module):
    ...
    def forward(self, hidden_states: Tensor) -> Tensor:
        ...
        with torch.no_grad():
            ...
            expert_hit = (
                torch.greater(expert_mask.sum(dim=(-1, -2)), 0).nonzero().reshape(-1)
            )
        ...
        for expert_idx in expert_hit:
            ...
            expert_output = self.experts[expert_idx_item](
                expert_input
            )  # [num_selected, D]
