...
from fairseq2.models.qwen_omni.config import QwenOmniConfig
...
def create_qwen_omni_model(config: QwenOmniConfig) -> TransformerLM:
    gangs = maybe_get_current_gangs()

    return QwenOmniFactory(config, gangs).create_model()


class QwenOmniFactory:
    def __init__(self, config: QwenOmniConfig, gangs: Gangs | None = None) -> None:
        self._config = config
        self._gangs = gangs
