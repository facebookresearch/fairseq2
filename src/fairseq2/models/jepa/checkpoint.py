...
def _convert_jepa_checkpoint(
    ...
    if not isinstance(encoder_checkpoint, dict):
        raise TypeError(
            f"The encoder state in `checkpoint` is expected to be of type `{dict}`, but is of type `{type(encoder_checkpoint)}` instead."
        )
...
def _convert_jepa_encoder_checkpoint(
    ...
    for name, param in checkpoint.items():
        if not isinstance(param, Tensor):
            raise TypeError(
                f"`checkpoint['encoder'][{name}]` is expected to be of type `{Tensor}`, but is of type `{type(param)}` instead."
            )
