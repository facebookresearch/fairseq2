...
class Gemma4Frontend(Module):
    ...
    def __init__(
        ...
        *,
        ...
        if ple_hidden_dim > 0 and vocab_size_per_layer_input > 0:
            ...
            if ple_norm is None:
                raise ValueError("`ple_norm` must be provided when PLE is enabled.")
    ...
    def _compute_ple(self, token_ids: Tensor, seqs: Tensor) -> Tensor:
        ...
        return (continuous + discrete) * scale  # type: ignore[operator]
            ...
            if n_frames >= n_slots:
                ...
                padded = torch.cat(
                    [
                        ...
                        audio_embeds.new_zeros(
                            n_slots - n_frames, audio_embeds.size(2)
                        ),
                    ],
                    dim=0,
                )
