...
"""Gemma 4 top-level decoder-only causal language model.

Combines :class:`~fairseq2.models.gemma4.frontend.Gemma4Frontend` (embeddings
+ optional PLE + optional audio injection) with
:class:`~fairseq2.models.gemma4.decoder.Gemma4Decoder` (decoder stack with
KV sharing) and a final projection (optionally tied to embeddings and/or
soft-capped).

Multimodal audio is supported when ``audio_tower`` and ``audio_embedder``
are provided.  Their outputs are merged into the input embeddings at
positions matching ``audio_token_id`` (resolved by the frontend).
"""
