...
class Gemma4ConformerSDPA(Module):
    ...
    def forward(
        ...
        *,
        ...
        query_valid = self._convert_to_block(
            seq_valid.unsqueeze(
                -1
            )  # [B, padded_len, 1] to match 3D for _convert_to_block
        ).squeeze(
            -1
        )  # [B, num_blocks, W]
        ...
        final_condition = (
            causal_condition.to(key_validity.device) & key_validity & query_validity
        )
    ...
    def _compute_relative_logits(
        ...
        sin_emb_heads = (
            projected.reshape(1, f_span, num_heads, head_dim).squeeze(0).float()
        )
