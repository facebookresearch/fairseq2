...
from torch.nn import Conv1d, GLU, Module, ModuleList, SiLU
    ...
    def __init__(
        ...
        *,
        ...
        self.inner_proj = Gemma4ClippedLinear(
            model_dim, inner_dim, bias=bias,
            use_clipping=use_clipping, device=device, dtype=dtype,
        )
        self.inner_activation = SiLU()
        self.output_proj = Gemma4ClippedLinear(
            inner_dim, model_dim, bias=bias,
            use_clipping=use_clipping, device=device, dtype=dtype,
        )
    ...
    def __init__(
        ...
        *,
        ...
        self.pointwise_conv1 = Gemma4ClippedLinear(
            model_dim, model_dim * 2, bias=False,
            use_clipping=use_clipping, device=device, dtype=dtype,
        )
        ...
        self.layer_norm = Gemma4AudioRMSNorm(
            model_dim, bias=False, eps=rms_norm_eps,
...
class Gemma4ConformerBlock(TransformerEncoderLayer):
    ...
    def _forward_self_attn(
        ...
        attn_bias_cache: AttentionBiasCache, mask: Tensor | None,
    ) -> Tensor:
        residual = seqs
        seqs = torch.clamp(seqs, -self.gradient_clipping, self.gradient_clipping)
    ...
    def _forward_conv(
        self, seqs: Tensor, seqs_layout: BatchLayout, mask: Tensor | None,
    ) -> Tensor:
        if mask is not None:
            validity = ~mask
...
def _build_conformer_block(
    ...
    *,
    ...
    ffn1_layer_norm = Gemma4AudioRMSNorm(
        config.hidden_size, bias=False, eps=config.rms_norm_eps,
        device=device, dtype=dtype,
    )
