...
@final
class QwenOmniMultiheadAttention(MultiheadAttention):
    """Represents QwenOmni MultiheadAttention as found on
    https://huggingface.co/Qwen/Qwen2.5-Omni-7B
    q and k norms have been removed in lieu of downstream
    LayerNorm. Positional encodings removed in lieu of
    upstream RoPE.
    """
        ...
        *,
        ...
        if out_proj is None:
            ...
            out_proj = Linear(
                ...
                model_dim,
                out_proj_bias,
                init_fn=out_proj_init_fn or init_mha_out_projection,
                device=device,
                dtype=dtype,
            )
        else:
            _warn_deprecated(
                "`out_proj` parameter of `QwenMultiheadAttention` is deprecated and will be removed in fairseq2 v0.12."
            )

            if out_proj_init_fn is not None:
                raise ValueError(
                    "`out_proj_init_fn` must not be specified when `out_proj` is specified."
                )

            if v_dim != out_proj.input_dim:
                raise ValueError(
                    f"`v_proj.output_dim` (or times the number of query groups when GQA) and `out_proj.input_dim` must be equal, but they are {v_dim} and {out_proj.input_dim} instead."
                )

            if out_proj.output_dim != model_dim:
                raise ValueError(
                    f"`out_proj.output_dim` must be equal to `model_dim` ({model_dim}), but is {out_proj.output_dim} instead."
                )
        ...
        if gangs is None or gangs.tp.size == 1:
            ...
            if isinstance(out_proj, Linear):
                self.out_proj = RowShardedLinear.from_linear(
                    out_proj, gangs.tp, scatter_input=False
                )
            else:
                self.ouput_proj = out_proj
...
def init_mha_out_projection(proj: Linear) -> None:
