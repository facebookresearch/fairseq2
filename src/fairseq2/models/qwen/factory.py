...
def create_qwen_model(
    resolver: DependencyResolver, config: QwenConfig
) -> TransformerLM:
    ...
    def create_final_projection(self, embed: Embedding) -> Projection:
        ...
        if config.tied_embeddings:
            if not isinstance(embed, StandardEmbedding):
                raise TypeError(
                    f"`embed` must be of type `{StandardEmbedding}` when `config.tied_embeddings` is set, but is of type `{type(embed)}` instead."
                )
