...
def _register_wav2vec2_configs(container: DependencyContainer) -> None:
    arch = ConfigRegistrar(container, Wav2Vec2Config)
    arch_encoder = ConfigRegistrar(
        container, Wav2Vec2EncoderConfig
    )  # TODO:cirquit changed from old context.get_config_registry(...).decorator, needs to be tested
