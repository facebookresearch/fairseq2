...
class RnntBeamSearchModule(nn.Module):
    ...
    def run_beam_search(self, encoder_out, encoder_lengths, nbest):
        ...
        while not done:
            ...
            force_blank_mask = torch.logical_or(
                n_emitted_symbols >= self.step_max_symbols, 
                torch.logical_not(active)
            )
            candidate_scores[force_blank_mask, :-1] = 2 * low_score
            ...
            nmerge_tokens[force_blank_mask[raveled_idx]] = self.blank  # Make sure one more time, to account for edge cases
