...
def _create_asset_download_manager(
    ...
    download_dir = asset_dirs.get_download_dir()

    noop_manager = NoopAssetDownloadManager()

    in_proc_manager = InProcAssetDownloadManager(download_dir)

    hg_hub = HuggingFaceHub()

    managers = {"file": noop_manager, "https": in_proc_manager, "hg": hg_hub}
