...
def create_asset_download_manager(resolver: DependencyResolver) -> AssetDownloadManager:
    ...
    noop_manager = NoopAssetDownloadManager()

    in_proc_manager = InProcAssetDownloadManager(asset_cache_dir)

    hg_manager = HuggingFaceHub()

    managers = {"file": noop_manager, "https": in_proc_manager, "hg": hg_manager}

    return CompositeAssetDownloadManager(managers)
