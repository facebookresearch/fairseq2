...
@final
class MaybeSystemAssetMetadataSource:
    def __init__(
    ...
    def maybe_load(self) -> AssetMetadataProvider | None:
        ...
        if path is not None:
            try:
                return self._metadata_loader.load(path)
...
@final
class MaybeUserAssetMetadataSource:
    def __init__(
        self, dirs: AssetDirectoryAccessor, metadata_loader: FileAssetMetadataLoader
    ) -> None:
        self._dirs = dirs
...
def register_assets(
    ...
    def load_metadata(resolver: DependencyResolver) -> AssetMetadataProvider | None:
        metadata_loader = resolver.resolve(FileAssetMetadataLoader)

        source = FileAssetMetadataSource(path, metadata_loader, not_exist_ok)

        return source.maybe_load()

    container.register(AssetMetadataProvider, load_metadata)


@final
class FileAssetMetadataSource:
    def __init__(
    ...
    def maybe_load(self) -> AssetMetadataProvider | None:
        try:
            return self._metadata_loader.load(self._path)
        except AssetSourceNotFoundError:
...
def register_package_assets(container: DependencyContainer, package: str) -> None:
    def load_metadata(resolver: DependencyResolver) -> AssetMetadataProvider:
        metadata_loader = resolver.resolve(PackageAssetMetadataLoader)

        source = PackageAssetMetadataSource(package, metadata_loader)
...
@final
class PackageAssetMetadataSource:
    def __init__(
    ...
    def load(self) -> AssetMetadataProvider:
        try:
            return self._metadata_loader.load(self._package)
        except OSError as ex:
            raise_operational_system_error(ex)
