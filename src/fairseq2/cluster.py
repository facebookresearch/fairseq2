...
def set_torch_distributed_env_variables(cluster: str = "auto") -> str:
    ...
    return handler.supported_cluster
...
class StandardClusterResolver(ClusterResolver):
    ...
    def resolve(self, name: str) -> ClusterHandler:
        ...
        if handler is None:
            raise ClusterNotSupportedError(name, self._handlers.keys())

        return handler


class ClusterNotSupportedError(Exception):
    def __init__(self, cluster: str, supported_clusters: Collection[str]) -> None:
        s = ", ".join(supported_clusters)

        super().__init__(
            f"{cluster} is not a supported cluster. Supported clusters are {s}."
        )
