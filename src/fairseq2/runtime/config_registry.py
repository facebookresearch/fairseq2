...
class AdvancedConfigSupplier(Protocol[ConfigT_co]):
...
ConfigDecorator: TypeAlias = Callable[
    [AdvancedConfigSupplier[ConfigT]], AdvancedConfigSupplier[ConfigT]
]
    ...
    def __call__(
        ...
        if resolver:
            ...
            def advanced_decorator(
                supplier: AdvancedConfigSupplier[ConfigT],
            ) -> AdvancedConfigSupplier[ConfigT]:
                self._container.register(self._kls, supplier, key=name)
...
@final
class StandardConfigProvider(ConfigProvider[ConfigT]):
    ...
    @override
    def get_config_names(self) -> Iterable[str]:
        return self._resolver.resolve_all(str, key=self._kls)  # type: ignore[arg-type]
