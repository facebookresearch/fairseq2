...
from typing import Any, Final, Protocol, TypeVar, cast, final
...
class StandardDependencyContainer(DependencyContainer, DependencyResolver):
    ...
    def _get_object(self, kls: type[T], registration: _Registration) -> T | None:
        ...
        if obj is not None and not isinstance(obj, kls):
            raise InternalError(
                f"Object in the container is expected to be of type `{kls}`, but is of type `{type(obj)}` instead."
            )
...
class _Registration:
    def __init__(
        ...
        if obj is _NOT_SET and factory is None:
            raise InternalError("Neither `obj` nor `factory` is specified.")
...
class DependencyProvider(Provider[T]):
    ...
    def get(self, key: Hashable) -> T:
        obj = self.maybe_get(key)
        if obj is None:
            raise LookupError(f"No `{self._kls}` found for {key}.")
