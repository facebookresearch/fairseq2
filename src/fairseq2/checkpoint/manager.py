...
from fairseq2.gang import Gangs, all_sum
from fairseq2.io import TensorDumper, TensorDumpError, TensorLoader, TensorLoadError
...
class FileCheckpointManager(CheckpointManager):
    ...
    def _begin_checkpoint(self, step_nr: int) -> None:
        ...
        if gangs.root.rank == 0:
            try:
                ...
                raise InfraError(
                    f"A system error has occurred while creating the temporary '{tmp_step_dir}' checkpoint directory of step {step_nr}. See the nested exception for details."
                ) from ex
    ...
    def _collect_trainer_state(
        ...
        if gangs.root.rank == 0:
            try:
                ...
                raise InfraError(
                    f"A system error has occurred while creating the '{file.parent}' directory of step {step_nr}. See the nested exception for details."
                ) from ex
    ...
    def _do_save_checkpoint(
        ...
        if not blocking:
            ...
            for kind, (file, state_dict) in state.items():
                ...
                except (ValueError, TypeError, RuntimeError, PickleError) as ex:
    ...
    def _save_state_files(self, step_nr: int, state: CheckpointState) -> None:
        for kind, (file, state_dict) in state.items():
            try:
                ...
                raise CheckpointSaveError(
                    step_nr, f"The '{file}' state file of step {step_nr} cannot be saved. See the nested exception for details."  # fmt: skip
                ) from ex
    ...
    def _commit_checkpoint(self, step_nr: int) -> None:
        ...
        if gangs.root.rank == 0:
            ...
            try:
                ...
                raise InfraError(
                    f"A system error has occurred while renaming the temporary '{tmp_step_dir}' checkpoint directory of step {step_nr}. See the nested exception for details."
                ) from ex
    ...
    def save_score(self, step_nr: int, score: float) -> None:
        ...
        if gangs.root.rank == 0:
            ...
            try:
                ...
                raise InfraError(
                    f"A system error has occurres while creating the '{scores_dir}' checkpoint scores directory. See the nested exception for details."
                ) from ex
            ...
            try:
                ...
                raise InfraError(
                    f"A system error has occurred while saving the score of step {step_nr} to the '{score_file}' file. See the nested exception for details."
                ) from ex
    ...
    def load_trainer_state(self, step_nr: int, trainer: Stateful) -> None:
        ...
        try:
            ...
            raise CheckpointLoadError(
                step_nr, f"The trainer state of step {step_nr} cannot be restored. See the nested exception for details."  # fmt: skip
            ) from ex
    ...
    def load_model_state(self, step_nr: int, model: Stateful) -> None:
        ...
        try:
            ...
            raise CheckpointLoadError(
                step_nr, f"The model state of step {step_nr} cannot be restored. See the nested exception for details."  # fmt: skip
            ) from ex
    ...
    def load_optimizer_state(self, step_nr: int, optimizer: Stateful) -> None:
        ...
        try:
            ...
            raise CheckpointLoadError(
                step_nr, f"The optimizer state of step {step_nr} cannot be restored. See the nested exception for details."  # fmt: skip
            ) from ex
    ...
    def load_data_reader_state(self, step_nr: int, data_reader: Stateful) -> None:
        ...
        try:
            ...
            raise CheckpointLoadError(
                step_nr, f"The data reader state of step {step_nr} cannot be restored. See the nested exception for details."  # fmt: skip
            ) from ex
    ...
    def _load_state_dict(self, step_nr: int, pathname: str) -> dict[str, object]:
        ...
        with load_with_sdp_gang(gangs):  # Required for `ShardedTensor`.
            ...
            try:
                ...
                raise CheckpointNotFoundError(step_nr) from ex
            except TensorLoadError as ex:
                raise CheckpointLoadError(
                    step_nr, f"The '{file}' state file of step {step_nr} cannot be loaded. See the nested exception for details."  # fmt: skip
                ) from ex
    ...
    def delete_checkpoint(self, step_nr: int) -> None:
        ...
        if gangs.root.rank == 0:
            ...
            try:
                ...
                raise InfraError(
                    f"A system error has occurred while deleting the temporary '{tmp_step_dir}' checkpoint directory of step {step_nr}. See the nested exception for details."
                ) from ex
            ...
            try:
                ...
                raise InfraError(
                    f"A system error has occurred while accessing the '{step_dir}' checkpoint directory of step {step_nr}. See the nested exception for details."
                ) from ex
            ...
            if step_dir_exists:
                try:
                    ...
                    raise InfraError(
                        f"A system error has occurred while deleting the '{step_dir}' checkpoint directory of step {step_nr}. See the nested exception for details."
                    ) from ex
            ...
            try:
                ...
                raise InfraError(
                    f"A system error has occurred while deleting the '{score_file}' score file of step {step_nr}. See the nested exception for details."
                ) from ex
    ...
    def get_step_numbers(self, *, exclude_model_only: bool = False) -> list[int]:
        ...
        try:
            ...
            raise InfraError(
                f"A system error has occurred while traversing the '{self._checkpoint_dir}' checkpoint directory. See the nested exception for details."
            ) from ex
    ...
    def _do_load_scores(self, step_nrs: list[int]) -> list[tuple[float, int]]:
        ...
        for step_nr in step_nrs:
            ...
            try:
                ...
                raise InfraError(
                    f"A system error has occurred while reading the score of step {step_nr} from the '{score_file}' file. See the nested exception for details."
                ) from ex
            ...
            try:
                ...
                raise CheckpointLoadError(
                    step_nr, f"The score of step {step_nr} cannot be parsed as a floating-point number. See the nested exception for details."  # fmt: skip
                ) from ex
