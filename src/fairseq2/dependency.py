...
class ObjectFactory(Protocol[T_co]):
...
class DependencyContainer(ABC):
    @abstractmethod
    def register(
        self, kls: type[T], factory: ObjectFactory[T], *, key: Hashable | None = None
    ) -> None: ...
...
class DependencyResolver(ABC):
    ...
    @abstractmethod
    def resolve_provider(self, kls: type[T]) -> Provider[T]: ...


class StandardDependencyContainer(DependencyContainer, DependencyResolver):
    _registrations: dict[tuple[type[object], Hashable], list[_Registration]]
    _frozen: bool
    ...
    def _register(
        self, kls: type, key: Hashable | None, registration: _Registration
    ) -> None:
        if self._frozen:
    ...
    def resolve(self, kls: type[T], *, key: Hashable | None = None) -> T:
        ...
        try:
            ...
            if key is None:
                msg = f"No registered factory or object can be found for `{kls}`."
            else:
                msg = f"No registered factory or object can be found for `{kls}` with key '{key}'."
    ...
    @override
    def resolve_provider(self, kls: type[T]) -> Provider[T]:
        return RuntimeObjectProvider(self, kls)
...
class _Registration:
    ...
    def __init__(
        self, obj: object = _NOT_SET, factory: ObjectFactory[object] | None = None
    ) -> None:
...
@final
class RuntimeObjectProvider(Provider[T]):
    ...
    @override
    def resolve(self, key: Hashable) -> T:
