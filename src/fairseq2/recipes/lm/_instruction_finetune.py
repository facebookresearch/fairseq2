...
class LMInstructionFinetuneConfig:
...
def register_lm_instruction_finetune_configs(context: RuntimeContext) -> None:
    registry = context.get_config_registry(LMInstructionFinetuneConfig)

    preset = registry.decorator

    @preset("llama3_1_instruct")
    def llama3_1_instruct() -> LMInstructionFinetuneConfig:
        config = LMInstructionFinetuneConfig()
        config.model_config = DropoutConfig()
        return config

    @preset("llama3_1_instruct_constant_lr")
    def llama3_1_instruct_constant_lr() -> LMInstructionFinetuneConfig:
    ...
    def llama3_1_instruct_lr_anneal_0() -> LMInstructionFinetuneConfig:
        config = llama3_1_instruct()
    ...
    def llama3_1_70b_instruct() -> LMInstructionFinetuneConfig:
        config = llama3_1_instruct()
    ...
    def llama2_7b_chat() -> LMInstructionFinetuneConfig:
        config = llama3_1_instruct()
    ...
    def llama2_70b_chat() -> LMInstructionFinetuneConfig:
...
def load_lm_instruction_finetuner(
    context: RuntimeContext, config: LMInstructionFinetuneConfig, output_dir: Path
    ...
    # Initialize the unit.
    unit = LMInstructionFinetuneUnit(criterion, dp_gang)
    ...
    if config.valid_split is not None:
        valid_unit = LMInstructionValidUnit(criterion, dp_gang)
...
@final
class LMInstructionFinetuneUnit(AbstractTrainUnit[SequenceBatch]):
    _criterion: LMInstructionFinetuneCriterion
    _metric_bag: SequenceMetricBag

    def __init__(self, criterion: LMInstructionFinetuneCriterion, gang: Gang) -> None:
...
@final
class LMInstructionValidUnit(AbstractEvalUnit[SequenceBatch]):
    _criterion: LMInstructionFinetuneCriterion
    _metric_bag: SequenceMetricBag
...
@final
class LMInstructionFinetuneCriterion:
    _model: Module
