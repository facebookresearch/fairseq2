...
from fairseq2.gang import Gangs, GangSetupError, setup_parallel_gangs, setup_root_gang
...
def setup_gangs(context: RuntimeContext, recipe_config: object) -> Gangs:
    ...
    try:
        gangs = setup_parallel_gangs(
            root_gang, tp_size=gang_section.tensor_parallel_size
        )
    except GangSetupError as ex:
        raise ProgramError(
