...
class RecipeCommandHandler(CliCommandHandler, Generic[ConfigT]):
    ...
    def run(self, args: Namespace) -> int:
        ...
        config_kls = self._preset_configs.config_kls

        config_reader = self._create_config_reader()

        cluster_registry = self._create_cluster_registry()

        sweep_tagger = self._create_sweep_tagger(args.no_sweep_dir)
        ...
        program = RecipeProgram(
            view, presets, self._default_preset, config_kls, config_reader, runner
        )
    ...
    @staticmethod
    def _create_view() -> RecipeProgramView:
        console = get_console()

        return StandardRecipeProgramView(console, dump_yaml)

    def _create_config_reader(self) -> ConfigReader:
        file_system = StandardFileSystem()

        return StandardConfigReader(self._preset_configs, file_system, load_yaml)

    @staticmethod
    def _create_cluster_registry() -> ClusterRegistry:
    ...
    def _create_sweep_tagger(self, no_sweep_dir: bool) -> SweepTagger:
        if no_sweep_dir:
...
class RecipeProgram(Generic[ConfigT]):
    ...
    def run(self, args: Namespace) -> None:
        if args.list_presets:
            self._view.print_presets(self._presets, self._default_preset)

            return

        try:
            unstructured_config = self._config_reader.read(
        ...
        if args.dump_config:
            self._view.print_config(unstructured_config)
        ...
        config = structure(unstructured_config, self._config_kls)
...
class RecipeProgramView(ABC):
    @abstractmethod
    def print_presets(self, presets: Set[str], default_preset: str) -> None:
        ...

    @abstractmethod
    def print_config(self, config: object) -> None:
        ...


@final
class StandardRecipeProgramView(RecipeProgramView):
    _console: Console
    _yaml_dumper: YamlDumper

    def __init__(self, console: Console, yaml_dumper: YamlDumper) -> None:
