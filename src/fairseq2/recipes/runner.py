...
from fairseq2.utils.structured import StructureError, merge_unstructured, unstructure
...
class StandardRecipeRunner(RecipeRunner[ConfigT]):
    ...
    def run(
        ...
        *,
        ...
        log_file = sweep_output_dir.joinpath("logs/rank_{rank}.log")

        self._logging_initializer.initialize(log_file, debug=debug)

        log.info("The log files stored under the '{}' directory.", log_file.parent)
        ...
        recipe = self._loader(config, sweep_output_dir)
