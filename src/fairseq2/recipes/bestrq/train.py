...
from fairseq2.recipes.bestrq.eval import BestRQEvalUnit
...
bestrq_train_presets = ConfigRegistry[BestRQTrainConfig]()

bestrq_train_preset = bestrq_train_presets.decorator
...
@bestrq_train_preset("large_960h")
def _large_960h() -> BestRQTrainConfig:
    config = BestRQTrainConfig()
...
def load_bestrq_trainer(
    config: BestRQTrainConfig, output_dir: Path
    ...
    criterion = BestRQCriterion(
        dp_model, config.label_smoothing
    )

    # Initialize the train unit.
    unit = BestRQTrainUnit(criterion, gang)
    ...
    valid_unit = BestRQEvalUnit(criterion, gang)
...
@final
class BestRQTrainUnit(AbstractTrainUnit[SequenceBatch]):
    ...
    def __init__(self, criterion: BestRQCriterion, gang: Gang) -> None:
        ...
        self._metric_bag = BestRQMetricBag(gang)
