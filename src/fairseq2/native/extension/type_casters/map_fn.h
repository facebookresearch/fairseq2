...
template <>
struct type_caster<fairseq2::map_fn> {
    PYBIND11_TYPE_CASTER(fairseq2::map_fn, const_name("Callable[[Any], Any]"));
...
public:
    ...
    static handle
    cast(const fairseq2::map_fn &fn, return_value_policy policy, handle)
    {
        return cpp_function{fn, policy}.release();
    }

    static handle
    cast(fairseq2::map_fn &&fn, return_value_policy policy, handle)
    {
