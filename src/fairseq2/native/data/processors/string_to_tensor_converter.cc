...
data
string_to_tensor_converter::process(data &&d) const
{
    if (!d.is_string())
        throw std::invalid_argument{"The input data must be of type string."};

    std::vector<immutable_string> strs = d.as_string().split(' ');

    auto dim = static_cast<std::int64_t>(strs.size());

    at::Tensor tensor = at::empty(dim, dtype_);

    AT_DISPATCH_INTEGRAL_TYPES(dtype_, "fill_storage", [&] {
        fill_storage<scalar_t>(tensor, strs);
    });

    if (size_)
        tensor = tensor.view(*size_);

    return data{std::move(tensor)};
}

template <typename T>
void
string_to_tensor_converter::fill_storage(
    at::Tensor &t, const std::vector<immutable_string> &strs) const
{
    std::int64_t idx = 0;

    auto accessor = t.accessor<T, 1>();

    for (const immutable_string &s : strs) {
        const char *end = s.data() + s.size();

        T parsed_value{};

        std::from_chars_result r = std::from_chars(s.data(), end, parsed_value);
