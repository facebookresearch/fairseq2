...
class Trainer(Task):
    ...
    def _maybe_restore_state(self) -> _TrainerState:
        ...
        self._checkpoint_manager.load_model_state(step_nr, self._unit.model_context)
    ...
    def _do_run(self) -> None:
        self._unit.model_context.model.train()
    ...
    def _do_run_step(self, progress_task: ProgressTask) -> _TrainerState:
        ...
        with self._compute_watch:
            ...
            # the backward pass.
            if num_targets > 0:
                normalize_grads(self._unit.model_context.model, gangs.dp, num_targets)

            self._loss_scaler.unscale_grads_()

            # Clip the gradients.
            with record_function(f"step_{step_nr}_grad_norm"):
                grad_norm = self._unit.model_context.clip_grad_norm(self._max_grad_norm)
    ...
    def _maybe_no_sync(self, batch_nr: int, num_batches: int) -> ContextManager:
        if self._no_sync_grad_accumulation:
            if batch_nr < num_batches - 1:
                return self._unit.model_context.no_sync()
    ...
    def _validate(self) -> float | None:
        ...
        with self._unit.model_context.summon_full_parameters():
