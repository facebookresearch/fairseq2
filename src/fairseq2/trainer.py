...
from fairseq2.checkpoint import CheckpointManager, HuggingFaceSaver, Stateful
        ...
        def load_stateful(name: str) -> None:
            ...
            if isinstance(value, Stateful):
                ...
                try:
                    ...
                    raise ValueError(
                        f"`state_dict['{name}']` is not a valid `{kls}` state."
                    ) from ex
            else:
                if type(state) != kls:
                    raise TypeError(
...
class MinimumLossScaleReachedError(Exception):
    ...
    def __init__(self, step_nr: int) -> None:
        super().__init__(f"Gradients are scaled down to minimum at step {step_nr}.")
