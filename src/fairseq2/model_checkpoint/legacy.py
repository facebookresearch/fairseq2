...
@final
class LegacyModelCheckpointLoader(ModelCheckpointLoader):
    ...
    def lazy_load(
        ...
        *,
        ...
        if not is_dir:
            msg = f"{path} does not point to a legacy fairseq2 checkpoint."

            raise ModelCheckpointError(path, msg)

        dp_files = self._get_checkpoint_files(path)

        dp_size = len(dp_files)
        ...
        # If the source and target parallel sizes match, avoid loading redundant
        # checkpoint files.
        if gangs.tp.size == 1 and gangs.sdp.size == dp_size:
            dp_files = [dp_files[gangs.sdp.rank]]

            source_shard_sizes = (1, 1)

            target_shard_sizes = (1, 1)
            target_shard_ranks = (0, 0)
        ...
        for dp_file in dp_files:
            ...
            try:
                dp_shard = cast(dict[str, object], dp_shard["model"])
            except KeyError:
                pass
        ...
        for key in keys:
            ...
            if dp_splits[0] in memo:  # Yield shared tensors only once.
                continue

            memo.add(dp_splits[0])
    ...
    def _get_checkpoint_files(self, path: Path) -> list[Path]:
        ...
        for dp_idx in count():
            dp_file = path.joinpath(f"shard.{dp_idx:02d}.pt")
    ...
    def supports_path(self, path: Path) -> bool:
        ...
        file = path.joinpath("shard.00.pt")
