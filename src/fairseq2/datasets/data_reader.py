    ...
    def __next__(self) -> list[BatchT]:
        ...
        for idx in range(self._num_accumulate):
            try:
                ...
                msg = f"Data pipeline failed to read the next batch from the {self._split} split of the {self._dataset_name} dataset."

                raise DataReadError(self._dataset_name, self._split, msg) from ex
