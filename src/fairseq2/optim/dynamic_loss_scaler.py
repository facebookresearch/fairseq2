...
class DynamicLossScaler:
            ...
            for group in optimizer.param_groups:
                for param in group["params"]:
                    if param.dtype != torch.float32 and param.dtype != torch.float16:
                        raise ValueError(
                            f"Parameters held by `optimizer` must be of type `torch.float32` or `torch.float16`, but it holds a parameter of type `{param.dtype}`."
                        )
    ...
    def load_state_dict(self, state_dict: dict[str, object]) -> None:
        ...
        try:
            ...
            raise ValueError(
                f"`state_dict['grad_scaler']` is not a valid `{type(self._grad_scaler)}` state."
            ) from ex
