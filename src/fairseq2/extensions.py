...
def run_extensions(name: str, *args: Any, **kwargs: Any) -> None:
    ...
    for entry_point in entry_points(group="fairseq2.extension"):
        try:
            ...
            if should_trace:
                raise ExtensionError(
                    f"The `{entry_point.value}` extension module has failed to load. See the nested exception for details."
                ) from ex
        ...
        try:
            ...
            if should_trace:
                raise ExtensionError(
                    f"The `{entry_point.value}.{name}` extension function has failed. See the nested exception for details."
                ) from ex
