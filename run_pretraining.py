    ...
    if preset.split("_")[-1] == "perf":
        ...
        for seed in (2, 3, 4):
            ...
            output_dir = root_dir.joinpath(
                f"wav2vec2_train_perf/fairseq2/{preset}_{num_gpus}gpu_seed{config.seed}"
            )
        ...
        cluster.run_job_array(train_wav2vec2_model, job_configs, job_output_dirs)
    else:
        config = wav2vec2_train_presets.get(preset)
        config.seed = args.seed
        output_dir = root_dir.joinpath(f"wav2vec2_train/{preset}_seed{config.seed}")
...
if __name__ == "__main__":
    parser = ArgumentParser(prog="pretrain_mms")
    parser.add_argument("--gpu", type=int, default=1)
    parser.add_argument("--seed", type=int, default=2)
    parser.add_argument("--preset", type=str, default="base_960h")
