...
class TestPackOp:
    ...
    def test_op_works_when_truncate_is_false(self) -> None:
        ...
        pipeline = (
            read_sequence(seqs)
            .pack(num_elements=7, max_seq_len=32, truncate=False, drop_remainder=False)
            .and_return()
        )
    ...
    def test_op_saves_and_restores_its_state(self) -> None:
        ...
        assert_equal(d["seqs"], torch.tensor([3, 0, 5]))  # type: ignore[index]
        ...
        assert_equal(d["seqs"], torch.tensor([9, 1, 2]))  # type: ignore[index]
        ...
        assert_equal(d["seqs"], torch.tensor([4, 0, 0]))  # type: ignore[index]
