...
class TestCatOp:
    def test_op_works(self) -> None:
        ...
        pipeline = DataPipeline.cat([pipeline1], [pipeline2]).and_return()
    ...
    def test_op_raises_error_when_one_of_the_pipelines_is_broken(self) -> None:
        ...
        with pytest.raises(
            ValueError,
            match=r"^At least one of the specified data pipelines is broken and cannot be used in cat\.$",
        ):
            DataPipeline.cat([pipeline1], [pipeline2]).and_return()
