...
class TestGetKvProjectionRole:
    ...
    def test_no_sharing(self) -> None:
        ...
        for i in range(num_layers):
            role = get_kv_projection_role(i, layer_types[i], num_layers, 0, layer_types)
    ...
    def test_e4b_sharing_pattern(self) -> None:
        ...
        roles = [
            get_kv_projection_role(
                i, layer_types[i], num_layers, num_shared, layer_types
            )
            for i in range(num_layers)
        ]
        ...
        for i in range(first_shared, num_layers):
            assert (
                roles[i] == KVProjectionRole.CONSUMER
            ), f"Layer {i} should be CONSUMER"
        ...
        for i in source_layers:
            assert (
                i < first_shared
            ), f"SOURCE layer {i} must be before boundary {first_shared}"
