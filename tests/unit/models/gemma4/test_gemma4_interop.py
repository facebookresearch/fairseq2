...
class TestGemma4HuggingFaceConverter:
    ...
    def test_to_hg_state_dict_all_keys_converted(self) -> None:
        ...
        for key in hg_state_dict:
            ...
            assert not key.startswith("decoder_frontend."), f"Unconverted fs2 key: {key}"
    ...
    def test_to_hg_state_dict_ple_keys(self) -> None:
        ...
        # Frontend PLE keys
        ple_frontend_keys = [
            k for k in hg_state_dict
            if "embed_tokens_per_layer" in k
            or "per_layer_model_projection" in k
            or "per_layer_projection_norm" in k
        ]
        assert len(ple_frontend_keys) >= 3, f"Missing PLE frontend keys: {ple_frontend_keys}"

        # Per-layer PLE keys
        ple_layer_keys = [
            k for k in hg_state_dict
            if "per_layer_input_gate" in k
            or "per_layer_projection" in k and "model_projection" not in k
