...
from fairseq2.models.gemma4.tokenizer import GEMMA4_CHAT_TEMPLATE
...
class TestSharderSpecs:
    ...
    def test_all_specs_are_valid_shardspec(self) -> None:
        ...
        for config_fn in [get_gemma4_e4b_config, get_gemma4_31b_config, get_gemma4_26b_a4b_config]:
            config = config_fn()
            specs = get_gemma4_shard_specs(config)
...
class TestChatTemplate:
    """Tests for the Gemma 4 chat template string."""

    def test_template_contains_user_role(self) -> None:
        """Chat template handles user role."""
        assert "<start_of_turn>user" in GEMMA4_CHAT_TEMPLATE

    def test_template_contains_model_role(self) -> None:
        """Chat template handles model role."""
        assert "<start_of_turn>model" in GEMMA4_CHAT_TEMPLATE

    def test_template_handles_assistant_alias(self) -> None:
        """Chat template treats 'assistant' as alias for 'model'."""
        assert "'assistant'" in GEMMA4_CHAT_TEMPLATE

    def test_template_contains_system_role(self) -> None:
        """Chat template handles system role."""
        assert "<start_of_turn>system" in GEMMA4_CHAT_TEMPLATE

    def test_template_has_bos(self) -> None:
        """Chat template starts with BOS token."""
        assert "bos_token" in GEMMA4_CHAT_TEMPLATE

    def test_template_has_generation_markers(self) -> None:
        """Chat template has generation markers for SFT masking."""
        assert "{%- generation %}" in GEMMA4_CHAT_TEMPLATE
        assert "{%- endgeneration %}" in GEMMA4_CHAT_TEMPLATE

    def test_template_has_generation_prompt(self) -> None:
        """Chat template supports add_generation_prompt."""
        assert "add_generation_prompt" in GEMMA4_CHAT_TEMPLATE
