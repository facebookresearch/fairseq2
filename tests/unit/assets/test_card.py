...
class TestAssetCard:
    ...
    def test_field_raises_error_when_field_does_not_exist(self) -> None:
        with pytest.raises(
            AssetCardFieldNotFoundError, match=r"^The 'test-card' asset card does not have a field named 'field9'\.$",  # fmt: skip
        ):
            self.card.field("field9").value()

    def test_as_raises_error_when_field_type_is_incorrect(self) -> None:
        with pytest.raises(
            AssetCardError, match=rf"The value of the 'field2' field of the 'test-card' asset card is expected to be of type `{bool}`, but is of type `{int}` instead\.$",  # fmt: skip
        ):
