...
MODEL_PATH = (
    "/storage/home/yunchaoyang1/models/Nemotron-3-Nano-Omni-30B-A3B-Reasoning-BF16"
)
RESULTS_DIR = Path(
    "/storage/home/yunchaoyang1/ProfAI/projects/12_NemotronH_fairseq2_Implementation/experiments"
)
...
def get_hf_logits(
    ...
    for i, prompt in enumerate(prompts):
        ...
        print(
            f"  Prompt {i+1}/{len(prompts)}: {prompt[:40]}... shape={outputs.logits.shape}"
        )
...
def compare_logits(
    ...
    for prompt in hf_results:
        ...
        assert (
            hf_logits.shape == fs2_logits.shape
    ...
    print("  Threshold: cos_mean >= 0.999, cos_min >= 0.998, top1 >= 0.95")
...
def main() -> None:
    ...
    if not Path(MODEL_PATH).exists():
        ...
        print(
            "Download with: huggingface-cli download nvidia/Nemotron-3-Nano-Omni-30B-A3B-Reasoning-BF16"
        )
        sys.exit(1)
