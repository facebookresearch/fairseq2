...
@pytest.mark.skipif(
    not _hf_model_type_available("qwen3_5"),
    reason="transformers does not support model_type 'qwen3_5' (upgrade transformers)",
)
class TestQwen35HFParity:
    """Numerical parity between HuggingFace and fairseq2 for Qwen 3.5 0.8B."""

    def test_logit_parity(self) -> None:
        ...
        hf_model.eval()
        print(
            f"  HF model loaded: {sum(p.numel() for p in hf_model.parameters()):,} params"
        )
        ...
        fs2_model.eval()
        print(
            f"  fs2 model built: {sum(p.numel() for p in fs2_model.parameters()):,} params"
        )
        ...
        if missing or unexpected:
            ...
            if result.missing_keys:
                pytest.fail(
                    f"Cannot proceed with {len(result.missing_keys)} missing keys: "
                    + ", ".join(sorted(result.missing_keys)[:30])
                )
        else:
        ...
        with torch.no_grad():
            ...
            fs2_logits = fs2_model(seqs, seqs_layout)
        ...
        print(
            f"  fs2 top-5: {fs2_top5} -> {[hf_tokenizer.decode([t]) for t in fs2_top5]}"
        )
        ...
        ATOL = 1e-4
        assert (
            full_max_diff < ATOL or cos_sim > 0.9999
        ), f"Parity check failed: max diff {full_max_diff:.2e}, cosine sim {cos_sim:.6f}"
