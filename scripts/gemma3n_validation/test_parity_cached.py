...
"""Full parity test using cached HuggingFace model."""
...
def main() -> None:
    print("="*80)
    print("GEMMA3N PARITY TEST (CACHED MODEL)")
    print("="*80)

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"\nDevice: {device}")

    # Load HuggingFace model from cache
    print("\n[1/5] Loading HuggingFace model from cache...")
    ...
    # Filter out audio/vision tower from unexpected
    text_only_unexpected = [k for k in unexpected if not k.startswith("model.audio_tower") and not k.startswith("model.vision_tower")]

    if text_only_unexpected:
        print(f"\n⚠️  Unexpected TEXT keys ({len(text_only_unexpected)}):")
        for key in sorted(text_only_unexpected)[:20]:
            print(f"    - {key}")
        if len(text_only_unexpected) > 20:
            print(f"    ... and {len(text_only_unexpected) - 20} more")
    else:
        print(f"\n✓ No unexpected text-only keys (ignoring {len(unexpected) - len(text_only_unexpected)} audio/vision keys)")

    if not missing and not text_only_unexpected:
        print("\n✓ Checkpoint loaded successfully (all text model keys matched)")
    ...
    with torch.no_grad():
        ...
        batch_layout = BatchLayout((input_ids.shape[0], input_ids.shape[1]), seq_lens, device=device)
