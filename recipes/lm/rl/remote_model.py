...
from .rewards.athene_reward import NoEnvAtheneRewardPipeline
from .config import RayActorConfig, VllmRayActorConfig, HFRayActorConfig
# from .rewards.general_verifier_reward import (
...
# @dataclass(kw_only=True)
# class VllmActorConfig(ActorConfig):
...
# @dataclass(kw_only=True)
# class HFActorConfig(ActorConfig):
#     pipeline_name: str = ""
...
class RemoteVllmModel(RemoteModel):
    def __init__(
        self,
        ray_actor_name: str,
        num_replicas: int,
    ...
    def setup_vllm_worker(self, ray_actor_name, vllm_engine_args, gangs: Gangs):
        ...
        llm = NoEnvLLM.options(
            ...
            worker_extension_cls="lm.rl.remote_model.WorkerExtension",
...
class RemoteHFModel(RemoteModel):
    def __init__(
        self,
        ray_actor_name: str,
        num_replicas: int,
        ...
        for replica_i in range(self.num_replicas):
            self.setup_replica(replica_i, tensor_parallel_size, pipeline_name)
...
def create_vllm_remote_model(
        gangs: Gangs, actor_config: VllmRayActorConfig
    ) -> Union[RemoteVllmModel, RemoteHFModel, None]:
    ...
    if gangs.dp.rank == 0 and gangs.tp.rank == 0:
        remote_model = RemoteVllmModel(
            ...
            actor_config.sync_every_n_steps,
...
def create_hf_remote_model(
        gangs: Gangs, actor_config: HFRayActorConfig
    ) -> Union[RemoteVllmModel, RemoteHFModel, None]:
    if gangs.dp.rank == 0 and gangs.tp.rank == 0:
        remote_model = RemoteHFModel(
                actor_config.ray_actor_name,
                actor_config.num_replicas,
