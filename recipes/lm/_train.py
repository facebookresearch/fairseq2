...
from fairseq2.datasets.jsonl_text import JSONL_TEXT_DATASET_FAMILY
...
context = load_recipe_context(config, output_dir)

def load_clm_trainer(context: RecipeContext, config: CausalLMTrainConfig) -> Trainer:
...
class CausalLMTrainRecipe(TrainRecipe):
    def __init__(self, config: CausalLMTrainConfig) -> None:
        self.config = config

    @override
    def prepare_model(self, context: RuntimeContext, model: Model) -> None:
        pass
    ...
    @override
    def create_train_unit(self, context: RecipeContext) -> TrainUnit:
        model = context.get_model(CausalLM)

        return CausalLMTrainUnit(model)

    @override
    def create_train_reader(self, context: RecipeContext) -> DataReader:
        config = self.config

        dataset = context.get_dataset(TextDataset)
