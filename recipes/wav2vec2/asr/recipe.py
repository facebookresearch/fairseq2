...
class Wav2Vec2AsrRecipe(TrainRecipe):
    ...
    def register(self, container: DependencyContainer) -> None:
        """Register ASR dataset family with dependency container."""
    ...
    def fix_wav2vec2_position_encoder_weights(module: Module) -> Module:
        """Convert wav2vec2 position encoder from weight_g/weight_v to single weight.

        The deprecated torch.nn.utils.weight_norm uses two weights for magnitude/direction (g/v)
        and for old checkpoints to be compatible with the new position encoder implementation.

        This is required because we're sharing the encoder with the ASR model.
    ...
    def prepare_model(self, context: RecipeContext, model: Model) -> Model:
        """This method is called before the trainer is created to prepare the model for training.
        In this case, we load a reference model (think checkpoint) to start the training with
        the encoder and encoder frontend from a pretrained model."""
    ...
    def create_trainer(self, context: RecipeContext) -> Trainer:
        """
        Create ASR trainer with pretrained model loading and encoder freezing.
        ...
        if config.dataset.train_split == None:
            raise ValueError(
        ...
        if config.dataset.valid_split is not None:
            ...
            for split in valid_splits:
                valid_unit = Wav2Vec2AsrEvalUnit(criterion)
...
class Wav2Vec2AsrTrainUnit(TrainUnit[Seq2SeqBatch]):
    ...
    def set_step_nr(self, step_nr: int) -> None:
        """
        Handle encoder freezing/unfreezing based on training step.

        Called at the start of every step in TrainUnit.
        """
