...
from fairseq2.config_registry import get_config_registry
...
@final
class Wav2Vec2AsrTrainRecipe(TrainRecipe):
    def __init__(self, context: RecipeContext, config: Wav2Vec2AsrConfig) -> None:
    ...
    def load_model(self) -> Model:
        ...
        if model.newly_initialized:
            pt_model = self.context.load_reference_model(config.pretrained_model)
...
#    @override
#    def has_static_model_graph(self) -> bool:
#        return self.config.trainer.freeze_encoder_for_n_steps == 0
    ...
    def load_trainer(self) -> Trainer:
        ...
        dataset = self.context.get_dataset()

        tokenizer = self.context.get_tokenizer()
        ...
        return self.context.create_trainer(
            unit, data_reader, valid_units, valid_data_readers, seed=seed
        )
...
def register_wav2vec2_asr_train_configs(context: RuntimeContext) -> None:
    registry = get_config_registry(context, Wav2Vec2AsrTrainConfig)
...
def register_wav2vec2_asr_train(container: DependencyContainer) -> None:
    context.register(Model, load_wav2vec2_asr_model)


def load_wav2vec2_asr_model(context: DependencyContext) -> Model:
    model = load_base_model(context, Wav2Vec2AsrModel)
...
#    checkpoint_manager = create_checkpoint_manager(context, gangs, output_dir)
    ...
    model = load_base_model(Wav2Vec2AsrModel, context)
    ...
    return create_trainer(
        context, unit, data_reader, valid_units, valid_data_readers
    )
